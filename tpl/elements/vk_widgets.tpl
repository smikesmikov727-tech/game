	...
	<div class="vk-widgets">
		...
		<div class="block">
			...
			{for($i=0; $i < count($vk_admins); $i++)}
				...
				<a title="Перейти в профиль Вконтакте" target="_blank" href="https://vk.ru/id{{$vk_admins[$i]}}" id="admin_widget{{$i}}">
					<img src="../files/avatars/no_avatar.jpg?v={cache}" alt="">
					<span>Загрузка...</span>
				</a>
				{else}
            ...
            {if($conf->widgets_type == 1 && count($vk_admins) > 0)}
	            <script>
	                get_vk_profile_info(
	                    {for($i=0; $i < count($vk_admins); $i++)}
	                        '{{$vk_admins[$i]}}',
	                        '#admin_widget{{$i}} img',
	                        '#admin_widget{{$i}} span',
	                        '{{$vk_admins[$i]}}'{if($i!=count($vk_admins)-1)},{/if}
			            {/for}
	                );
	            </script>
            {/if}
		...
		{if($conf->vk_group == 1)}
			{if($conf->widgets_type == 1)}
				<script type="text/javascript" src="//vk.ru/js/api/openapi.js?120"></script>
