<tr id="sid{id}"
    ...
    data-toggle="modal"
        {if({type} == 7)}
			onclick="load_wstats({id},{server});load_mstats({id},{server});"
        {/if}
>
	<td>{place}</td>
	<td>{nick}</td>
			...
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-body" id="statinfo{id}">
							...
							<div class="table-responsive">
								<table class="table table-bordered">
									...
									<tr>
										<td><b>Операции</b></td>
										<td>
											<button class="btn btn-sm btn-outline-primary" onclick="dell_user_stats({id}, {server})">
												Удалить
											</button>
										</td>
									</tr>
						...
						<div class="table-responsive">
							<table class="table table-bordered">
								...
								<tr>
									<td><b> Ник </b></td>
                                    {if({type} == 7)}
										<td title="{name_history}"><b>{nick}</b> {nick_count}</td>
                                    {else}
										<td><b>{nick}</b></td>
                                    {/if}
								</tr>
                                {if(in_array({type}, array(1,2,3,5,6,7)))}
									<tr>
										<td><b> Steam ID </b></td>
										<td>{authid}</td>
									</tr>
                                ...
                                {/if}
                                {if(in_array({type}, array(1,2,3,7)))}
									<tr>
										<td><b> Урон </b></td>
										<td>{damage}</td>
									</tr>
                                ...
                                {/if}
                                {if(in_array({type}, array(1,2,3,5,7)))}
									<tr>
							...
							<div class="table-responsive mt-3">
								<table class="table table-bordered">
                                    ...
                                    {if({type} == 7)}
										<tr>
											<td><b> Уровень </b></td>
											<td>{level_rank} из {level_rank_total}</td>
										</tr>
                                    {/if}
                                    {if({type} == 2)}
									<tr>
                        ...
                        {if({type} == 7)}
							<h4 class="modal-title">Используемое оружие</h4>
							<div class="table-responsive mt-3" style="font-size: 90%">
								<table class="table table-bordered">
									<thead>
									<tr>
										<td>Оружие</td>
							...
							<h4 class="modal-title">Статистика по картам</h4>
							<div class="table-responsive mb-0 mt-3" style="font-size: 90%">
								<table class="table table-bordered">
									<thead>
									<tr>
										<td>Карта</td>
